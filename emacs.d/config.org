...
#+BEGIN_SRC emacs-lisp
  ...
  (defun now (&optional return-date-only)
    (interactive "P")
    (message (format-time-string (if return-date-only "%Y-%m-%d" "%Y-%m-%d %H:%M:%S%z"))))
...
#+begin_src emacs-lisp
  (when (not (getenv "CLOUDSYNC"))
    (setenv "CLOUDSYNC"
  	    (concat (file-name-as-directory (getenv "HOME"))
  		    "cloudsync")))

  (defvar CLOUDSYNC-NOTE-DIR
    (concat (file-name-as-directory
  	   (getenv "CLOUDSYNC")) "main/note/org"))

  (setq EMACS.D-DIR (path-join "~" ".emacs.d"))

  (let ((nix-bin (expand-file-name "~/.nix-profile/bin")))
    (unless (string-match-p (regexp-quote nix-bin) (getenv "PATH"))
...
#+begin_src emacs-lisp
  ;; this changes the org-mode styling
...
ref https://www.shaneikennedy.xyz/blog/configuring-emacs-from-scratch-in-2025
...
#+begin_src emacs-lisp
  ...
  (let ((bootstrap-file
    ...
    (unless (file-exists-p bootstrap-file)
      ;; Confirmation Check
      (when (y-or-n-p "straight.el bootstrap file not found. Download and install it now? ")
	(with-current-buffer
