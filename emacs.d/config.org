...
(use-package markdown-mode
  ...
  (custom-set-faces
   ...
   '(markdown-header-face-1 ((t (:inherit markdown-header-face :height 1.5 :weight bold :foreground "MidnightBlue"))))
   ;; Level 2 Heading: Slightly smaller and bold.
   '(markdown-header-face-2 ((t (:inherit markdown-header-face :height 1.3 :weight bold :foreground "DarkGreen"))))
   ;; Level 3 Heading: Standard size, bold, and distinct color.
   '(markdown-header-face-3 ((t (:inherit markdown-header-face :weight bold :foreground "Firebrick"))))
   ...
   '(markdown-inline-code-face ((t (:background "#EFEFEF" :foreground "#8B008B" :box (:line-width 1 :style released-button)))))

   ;; Block Code (Fenced Code Blocks): Use a distinct background color.
   '(markdown-code-face ((t (:background "#F5F5F5" :foreground "#333333"))))

   ;; Blockquotes: Add a border or distinct color for quotes.
   '(markdown-blockquote-face ((t (:foreground "DarkSlateGray" :slant italic :background "#F7F7F7"))))
   )

  ;; Enable pretty-code-blocks for better looking fenced code blocks.
  (add-hook 'markdown-mode-hook
            (lambda ()
              (setq tab-width 2) ; Optional: Set a preferred tab width for markdown.
              (markdown-pretty-code-blocks-mode))) ; This makes fenced code blocks look cleaner.
  )
#+end_src
...
** org-logseq
;;; MIGRATED: org-logseq + fd-query/open-link overrides → my-org.el
...
#+begin_src emacs-lisp
  ...
  (defun my/org-logseq-auto-enable ()
    "Enable `org-logseq-mode' if the current Org file is located within
     the directory specified by `org-logseq-dir'."
    (interactive)
...
** win-switch
;;; MIGRATED: win-switch → my-packages.el
...
;;; MIGRATED: paredit hooks (emacs-lisp/lisp/lisp-interaction/clojure/clojurescript) → my-packages.el
;;; PENDING:  hy-mode/sibilant-mode paredit hooks — evaluate if needed
...
* obsidian.el
;;; MIGRATED: obsidian → my-packages.el (obsidian-directory replaces obsidian-specify-path)
...
#+begin_src emacs-lisp
  (use-package obsidian
    ...
    :demand t
    :config
    (obsidian-specify-path cloudsync-note-dir)
...
* paredit duplicate expression
;;; MIGRATED: paredit-duplicate-after-point + C-S-d binding → my-packages.el
...
#+begin_src emacs-lisp
(straight-use-package
 '(cond-let :repo "emacsmirror/cond-let" :host github :branch "master"))

;; you MAY need this. transient is built in but not new enough
;; for claude-code-ide. i had to
;; (unload-feature 'transient t)
;; and load this stuff again
(use-package transient
  :straight t
  :config
  ;; You can add any required configuration here, but often none is needed.
  (message "Loaded modern transient"))
...
#+begin_src emacs-lisp
  ...
  (exec-path-from-shell-setenv
   "PATH"
   (substring (mapconcat (lambda (x) (concat ":" x))
                         (list (file-name-directory "/nix/store/91y8h7rysgi5cl8hsj2bmjd00dq2l9pk-nodejs-22.14.0/bin/node")
                               "/opt/npm/bin"
                               (getenv "PATH"))) 1))
#+end_src
