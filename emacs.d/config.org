...
#+begin_src emacs-lisp
  (local--load-packages
   ...
   free-keys  ;; show unbound keys; previously used "unbound" package
...
#+BEGIN_SRC emacs-lisp
  (quote local--load-packages
   ...
   vterm
   websocket
   which-key  ;; repo is archived; likely bundled in emacs 30
...
#+begin_src emacs-lisp
  (defvar CLOUDSYNC-NOTE-DIR (concat (file-name-as-directory
                                      (or (getenv "CLOUDSYNC")
                                          (concat (getenv "HOME") "/cloudsync"))) "main/note/org"))
#+end_src
